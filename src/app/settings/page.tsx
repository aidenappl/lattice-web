...
import type { WebhookConfig, GlobalEnvVar, Template } from "@/types";
import {
  reqUpdateAPI,
  reqUpdateWeb,
  reqRefreshVersions,
...
function GlobalEnvVarsSection({ adminUser }: { adminUser: boolean }) {
  const showConfirm = useConfirm();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      ...
      if (res.success) {
        ...
        setError("error_message" in res ? res.error_message : "Failed to update environment variable");
      }
    } else {
      ...
      if (res.success) {
        ...
        setError("error_message" in res ? res.error_message : "Failed to create environment variable");
      }
    }
    setSubmitting(false);
  };
  ...
  const handleDelete = async (ev: GlobalEnvVar) => {
    const ok = await showConfirm({
      ...
      message: `Are you sure you want to delete "${ev.key}"? This action cannot be undone.`,
      confirmLabel: "Delete",
      variant: "danger",
    });
    if (!ok) return;
    ...
    if (res.success) {
      ...
      toast.error("error_message" in res ? res.error_message : "Failed to delete environment variable");
    }
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-sm font-semibold text-primary">Global Environment Variables</h2>
        {adminUser && (
          <Button
            onClick={() => {
              if (showForm) resetForm();
        ...
        <form onSubmit={handleSubmit} className="card p-6 mb-6 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Input
              ...
              placeholder={editingId && isSecret ? "Enter new value to change" : "Value"}
              value={value}
          ...
          <Button
            type="submit"
            disabled={submitting || (!editingId && (!key.trim() || !value.trim())) || (!!editingId && !value.trim() && isSecret === envVars.find(v => v.id === editingId)?.is_secret)}
          >
            {submitting
              ? editingId ? "Updating..." : "Creating..."
