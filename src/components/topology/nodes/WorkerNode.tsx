...
function WorkerNodeComponent({ data }: { data: WorkerNodeData }) {
  ...
  return (
    <div className="relative w-full h-full">
      {/* Pulse wave rings — only when heartbeat is active */}
      {data.recentHeartbeat && (
        <>
          <div className="absolute inset-0 rounded-xl topology-pulse-ring" />
          <div className="absolute inset-0 rounded-xl topology-pulse-ring topology-pulse-ring-delayed" />
        </>
      )}
      <div
        className={`relative rounded-xl border bg-surface-alt px-5 py-4 w-full h-full cursor-pointer transition-all duration-300 hover:shadow-lg ${borderClass}`}
      >
        <Handle
          type="target"
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              ...
              <span
                className={`h-2 w-2 rounded-full shrink-0 ${dotClass} ${data.status === "online" ? "animate-pulse" : ""}`}
              />
            </div>
